                        ...
                        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                            if (strlen($facebook) <= 100) {
                                if (strlen($instagram) <= 100) {
                                    ...
                                    header('Location: addClient.php?reg_err=instagram');
                                    die();
                                }
                            } else {
                                header('Location: addClient.php?reg_err=facebook');
                                die();
                            }
                        } else {
                            header('Location: addClient.php?reg_err=email');
                            die();
                        }
                    } else {
                        header('Location: addClient.php?reg_err=email_length');
                        die();
                    }
                } else {
                    header('Location: addClient.php?reg_err=tel');
                    die();
                }
            } else {
                header('Location: addClient.php?reg_err=nom');
                die();
            }
        } else {
            header('Location: addClient.php?reg_err=prenom');
            die();
        }
    } else {
        header('Location: addClient.php?reg_err=already');
        die();
    }
}
